...
\begin{itemize}
    ...
    \item \textbf{\textit{Firebase}}: plataforma de \textit{backend} como serviço (\textit{BaaS}), utilizada para autenticação de usuários (\textit{Firebase Authentication}), banco de dados em tempo real e \textit{NoSQL} (\textit{Cloud Firestore}) e funções \textit{serverless} (\textit{Cloud Functions}) escritas em \textit{Node.js};
    \item \textbf{\textit{IMDb API}}: \textit{API} externa utilizada para obtenção de metadados sobre filmes e séries, enriquecendo as recomendações com informações detalhadas;
